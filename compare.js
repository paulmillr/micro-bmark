...
async function compare(title, dimensions, libs, opts) {
    const { libDims = ['name'], defaults = {}, dims, filter, filterObj = () => true, jsonOnly, dryRun, patchArgs, // patch arguments (very hacky way for decryption)
    ...
    main: while (true) {
        ...
        if (filterValues(curValues, filter)) {
            ...
            if (lib !== undefined && filterObj(obj)) {
                ...
                if (jsonOnly)
                    res.push({ ...obj, stats, perSec });
                else {
                    prevValues = printRow(curValues.concat([
    ...
    if (jsonOnly) {
        console.log(JSON.stringify({ name: title, data: res }, (k, v) => {
            if (typeof v === 'bigint')
