...
exports.compareMatrix = compareMatrix;
const red = '\x1b[31m';
const green = '\x1b[32m';
const blue = '\x1b[34m';
const cyan = '\x1b[36m';
...
function calcStats(list) {
    ...
    const formatted = `${red}± ${rme.toFixed(2)}% (${formatD(min)}..${formatD(max)})${reset}`;
...
function parseMatrix(matrix) {
  ...
  function helper(currentKeyArr, currentLabelArr, currentValueArr, index) {
    if (index === matrix.length) {
      result.push([currentKeyArr.join(' '), currentLabelArr, currentValueArr]);
      return;
    }

    for (const key in matrix[index]) {
      const newKeyArr = [...currentKeyArr, key];
      const newLabelArr = [...currentLabelArr, key];
      const newValueArr = [...currentValueArr, matrix[index][key]];
      helper(newKeyArr, newLabelArr, newValueArr, index + 1);
    }
  }
...
async function compareMatrix(title, matrix, libs) {
  console.log(title);
  const parsed = parseMatrix(matrix);
  for (const item of parsed) {
    const [title, titleParts, opts] = item;
    let samples = libs.samples || 10;
    for (const [lib, fn] of Object.entries(libs)) {
      if (lib === 'options' || lib === 'samples') continue;
      const prefix = `${title} `;
      let currSamples = (typeof samples === 'function') ? samples(...opts, lib) : samples;
      await mark(`  ${prefix}${cyan}${lib}${reset}`, currSamples, () => fn(...opts));
    }
    console.log();
  }
}
